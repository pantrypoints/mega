<script lang="ts">
  ...
  // Action function for auto-submit
  ...
  function handleAnswer(value: number) {
    ...
    if (currentIndex < QUESTIONS.length - 1) {
      ...
      // Auto-submit after a brief delay to show the "Finalizing" message
      setTimeout(() => {
        if (formElement) {
...
<!--         <p class="text-lg text-gray-600">
  ...
  <div class="w-full max-w-xl bg-white rounded-3xl shadow-2xl overflow-hidden min-h-[500px] flex flex-col relative">
    {#if isSubmitting && !form?.success}
      <div 
        class="absolute inset-0 z-50 bg-white flex flex-col items-center justify-center p-8 text-center" 
        in:fade
      >
        <Loader2 class="w-12 h-12 text-teal-500 animate-spin mb-4" />
        <h2 class="text-2xl font-bold text-gray-800">Finalizing...</h2>
        <p class="text-gray-500 text-sm">Averaging your personality profile.</p>
      </div>
    {/if}
    ...
    {#if !form?.success}
      ...
      <div class="p-8 flex-grow flex flex-col justify-center">
        ...
        <div class="flex justify-between items-center mb-8">
          <button 
            ...
            class="text-gray-400 hover:text-teal-600 disabled:opacity-0 transition-all p-2 rounded-lg hover:bg-gray-50"
            type="button"
          >
      ...
      <!-- Manual submit button (optional, hidden) -->
      <button 
        form="quiz-form" 
        type="submit" 
        id="auto-submit" 
    ...
    {:else}
      ...
      <div class="p-10 text-center flex flex-col items-center" in:fade>
        ...
        <p class="text-gray-500 mb-6">
          Your data has been merged into your user profile.
        </p>
