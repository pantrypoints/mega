...
<div class="min-h-screen bg-sky-50 dark:bg-slate-950 flex items-center justify-center p-4 transition-colors">
  <div class="w-full max-w-3xl bg-white dark:bg-slate-900 p-8 sm:p-10 rounded-3xl shadow-2xl border-t-4 border-sky-500 dark:border-sky-600 transition-colors">
    ...
    <a href="/products/{product.id}" class="inline-flex items-center text-sky-600 dark:text-sky-400 hover:text-sky-800 dark:hover:text-sky-300 transition mb-6 font-medium">
    ...
    <p class="text-center text-gray-500 dark:text-slate-400 mb-8">Update your product details</p>
    ...
    <form method="POST" use:enhance class="space-y-8">
      ...
      <div class="flex gap-4 pt-4">
        <button 
          ...
          class="flex-1 bg-teal-500 dark:bg-teal-600 hover:bg-teal-600 dark:hover:bg-teal-700 text-white font-bold py-3 rounded-xl shadow-lg transition duration-300 transform hover:scale-105 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
        >
          <Save class="w-5 h-5"/> {uploadingIndices.size > 0 ? 'Uploading...' : m.save_changes()}
        </button>

        <a href="/products/{product.id}"
          class="flex-1 bg-gray-200 dark:bg-slate-700 hover:bg-gray-300 dark:hover:bg-slate-600 text-gray-700 dark:text-slate-200 font-bold py-3 rounded-xl shadow-lg text-center transition duration-300">
          {m.cancel()}
        </a>
      </div>
    </form>
