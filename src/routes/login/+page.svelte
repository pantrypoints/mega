<script lang='ts'>
    ...
    function handleResetPassword() {
        ...
        closeModal();
        // You might want to redirect to a reset password page or show a success message
...
{#if isModalOpen}
<div 
    class="fixed inset-0 bg-gray-900 dark:bg-gray-950 bg-opacity-70 dark:bg-opacity-80 flex items-center justify-center z-50 transition-opacity"
    role="dialog"
    aria-modal="true"
    onclick={closeModal}
...
>
    <div 
        class="bg-white dark:bg-slate-900 p-8 rounded-xl shadow-2xl max-w-sm w-full transform transition-all duration-300 scale-100"
        role="document"
        onclick={(e) => e.stopPropagation()}
        onkeydown={(e) => e.stopPropagation()}
    >
        <!-- Modal content -->
        <h2 class="text-xl font-bold text-gray-800 dark:text-white mb-4">{m.gotopointsbanker()}</h2>
        <p class="text-gray-600 dark:text-slate-400 mb-6">{m.ppismeetup()}</p>

        <div class="flex gap-3 mb-4">
            <a href="/banker"
                class="flex-1 bg-gray-200 dark:bg-teal-700 hover:bg-teal-300 dark:hover:bg-teal-600 text-center text-teal-700 dark:text-slate-200 font-semibold py-2 rounded-lg transition">
                Points Banker Details
            </a>            
        </div>                   
        <div class="flex gap-3 mb-4">
            <button 
                onclick={closeModal}
                class="flex-1 bg-gray-200 dark:bg-slate-700 hover:bg-gray-300 dark:hover:bg-slate-600 text-gray-700 dark:text-slate-200 font-semibold py-2 rounded-lg transition">
                Close
            </button> 
        </div>
                    ...
                    >
                        {#if passwordVisible}
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.638 0-8.406-2.583-11.85-6.071m-.151-.013L2.43 11.2m.112-1.127l1.011-1.011m1.127-.112l1.011-1.011m1.127-.112l1.011-1.011m1.127-.112l1.011-1.011m1.127-.112l1.011-1.011m1.127-.112l1.011-1.011m1.127-.112l1.011-1.011M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
